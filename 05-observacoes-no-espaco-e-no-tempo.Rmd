...
determinado conjunto de dados não seja produzido com esse propósito, seu reuso futuro depende grandemente do 
...
determinantes ou úteis para explicar as características do solo. Estão incluídas aí informações sobre o 
...
As observações espaço-temporais do solo também costumam ser acompanhadas por informações pontuais sobre o 
próprio solo e que refletem sua relação com o ambiente. Tais informações expressam as características físicas,
químicas e (micro)biológicas das diferentes camadas do solo. Por exemplo, a condição de drenagem, a presença
de pedregosidade e rochosidade, a classificação taxonômica, e a própria profundidade do solo. Na verdade, 
muitos conjuntos de dados possuem apenas esse tipo de informação, geralmente quanto de cunho exploratório, 
quando não são coletadas amostras das camadas do solo. Essas informações também são inseridas na tabela
*observacao*.

A tabela *observacao* está organizada de maneira a armazenar os dados em colunas. Assim, à cada observação do 
solo é dedicada uma linha exclusiva, e os dados adicionados sequencialmente nas colunas. Como os dados 
coletados durante uma observação do solo podem ser muitos e diversos, não há imposição limites quanto ao 
número de colunas. O único requerimento é que sejam preenchidas todas as colunas obrigatórias e que seja 
usada nomeclatura padronizada para as demais colunas. A primeira linha da tabela *observacao* contém o nome 
das colunas. Já a segunda linha da tabela *observacao* fica reservada para registrar as unidades de medida, 
por exemplo, para variáveis como a profundidade do solo.
