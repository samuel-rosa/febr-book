...
A segunda parte, `<extração>`, refere-se ao método usado para extraír o elemento em questão do material do solo e trazê-lo para a suspensão que será usada para determinar sua concentração no solo. O método usado para realizar essa determinação é identificado usando a terceira parte do nome do campo identificador, `<determinação>`. Por exemplo, `fe_cloridrico_eaa` refere-se ao conteúdo de ferro (`fe`) determinado por espectrofotometria de absorção atômica (`eaa`) no extrato de solução de ácido clorídrico (`cloridrico`).
